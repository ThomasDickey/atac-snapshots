...
static char const atactm_c[] = "$Id: atactm.c,v 3.10 2013/12/09 00:36:01 tom Exp $";
...
usage(char *cmd)
{
    ...
    fprintf(stderr,
	    "%s -c cost -n testname [-x] [-o new.trace] [file.trace]\n", cmd);
    ...
    fprintf(stderr, "%s -L [-n testnames [-x]] [file.trace]\n", cmd);
    fprintf(stderr,
	    "%s -r newname [-n testnames [-x]] [-o new.trace] [file.trace]\n", cmd);
...
*/
static char *
prefixFileName(const char *filename,
	       const char *prefix)
{
    const char *f;
    const char *p;
     ...
     char *argv[])
{
	...
	while (*++p)
		...
		if (names == NULL) {
		    ...
		    names = (char *) realloc(names, strlen(names) +
					     strlen(p) + 2);
		    strcat(names, ",");
		    strcat(names, p);
		}
