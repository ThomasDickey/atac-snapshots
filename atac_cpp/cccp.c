...
static char cccp_c[] = 
	"$Header: /users/source/archives/atac.vcs/atac_cpp/RCS/cccp.c,v 3.11 1994/08/03 10:33:06 saul Exp $";
/*
*-----------------------------------------------$Log: cccp.c,v $
*-----------------------------------------------Revision 3.11  1994/08/03 10:33:06  saul
*-----------------------------------------------FROM_KEYS
*-----------------------------------------------
* Revision 3.11  94/08/03  10:33:06  saul
* Fix free loop that never terminates on linux
* 
* Revision 3.10  93/08/11  14:42:37  saul
* #pragma rejected by some cpp's
* 
* Revision 3.9  93/08/11  10:03:28  saul
* Get rid of ...TIME_H_SUPPORT (also USE_C_ALLOCA)
* 
* Revision 3.8  93/08/05  14:10:01  saul
* extraneous code in newline_fix
* MVS character classifcation code errors
* 
* Revision 3.7  93/08/04  15:37:54  ewk
* Added MVS and solaris support.  Squelched some ANSI warnings.
* 
* Revision 3.6  93/07/09  10:17:49  saul
* VMS and MVS changes.
* 
* Revision 3.5  93/06/07  14:08:45  saul
* Supply forward declaration of getMtime for portability.
* 
* Revision 3.4  93/04/29  17:07:54  ewk
* Changed to use malloc rather than alloca when
* compiled with the ATAC_MODS compilation directive set.
* 
* Revision 3.3  92/12/31  13:18:12  saul
* Bad fix
* 
* Revision 3.2  92/12/31  13:06:16  saul
* Repeated error messages when predefs file is empty.
* 
* Revision 3.1  92/12/02  11:35:25  saul
* Clean up invalid option checking.
* 
* Revision 3.0  92/11/06  07:46:40  saul
* propagate to version 3.0
* 
* Revision 2.12  92/11/03  10:11:31  saul
* .
* 
* Revision 2.11  92/11/03  08:03:01  saul
* revise time.h includes
* 
* Revision 2.10  92/11/02  15:47:01  saul
* include time.h or sys/time.h, not both
* 
* Revision 2.9  92/11/02  11:41:06  saul
* use portable.h to determine portability issues
* 
* Revision 2.8  92/10/30  09:51:50  saul
* include portable.h
* 
* Revision 2.7  92/10/28  09:10:46  saul
* new -atac_noexpand option to suppress @<...@|...@>
* new -atac_predefs option to supply predef symbols in a file
* 
* Revision 2.6  92/09/08  07:50:57  saul
* -% option protects selected macros from redefinition (for putc,getc).
* 
* Revision 2.5  92/04/10  13:17:05  saul
* typecaste needed for enum used as int for some compilers
* 
* Revision 2.4  92/04/08  11:50:20  saul
* make __STDC__ undefined if STDC_VALUE==0
* 
* Revision 2.3  92/04/06  12:47:53  saul
* GNU version 1.40 + ATAC_EXPAND + ATAC_LINENO
* 
* Revision 2.2  91/06/19  13:49:33  saul
* add ifdef for aix include.
* 
* Revision 2.1  91/06/19  13:45:29  saul
...
#define S_ISREG(m)	(0)
static initialize_char_syntax ();
static initialize_builtins ();
#endif /* MVS */
...
#ifdef SYS_RESOURCE_H_SUPPORT
/* #include <sys/resource.h>
*/
#endif /*  SYS_RESOURCE_H_SUPPORT */
...
#define GCC_INCLUDE_DIR CC_INCLUDE_DIR
#endif
...
int error (), error_from_errno (), warning (), error_with_line (),
    line_for_error ();
  ...
  if (indepth >= (INPUT_STACK_MAX - 1))					\
    {									\
      error_with_line (line_for_error (instack[indepth].lineno),	\
		       "macro or #include recursion too deep", 0, 0, 0);\
      code;								\
    }
     ...
     char **argv;
{
  ...
  for (i = 1; i < argc; i++) {
	...
	if (argv[i][2] != 0)
	  ...
	  fatal ("Filename missing after -i option", 0);
	else
      ...
      case '%':
	{
	  ...
	  if (argv[i][2] != 0)
	    ...
	    fatal ("Macro name missing after -% option", 0);
	  else
	...
	if (strcmp(argv[i], "-atac_predefs") == 0) {
	    if (i + 1 == argc) {
		fatal ("Filename missing after -predefs option", 0);
		break;
	    }
	    predefFile = argv[++i];
	    break;
  ...
  if (!inhibit_predefs) {
#ifdef ATAC_NOALLOCA
    char *p = (char *) xmalloc (strlen (predefs) + 1);
    char *memptr = p;
#else /* ATAC_NOALLOCA */
	...
	*p++= 0;
	...
	if (sbuf.st_size != 0) {
	    f = fopen(predefFile, "r");
	    if (f == NULL)
		pfatal_with_name(predefFile);
	    buf = (char *)xmalloc(sbuf.st_size + 1);
	    while (fgets(buf, sbuf.st_size, f) != NULL) {
...
#else /* MVS */
    if (fwrite (outbuf.buf, outbuf.bufp - outbuf.buf, 1, stdout) < 1)
#endif /* MVS */
     ...
     FILE_BUF *buf;
{
  ...
  if (warn_trigraphs && fptr != bptr)
    warning ("%d trigraph(s) encountered", (fptr - bptr) / 2, 0, 0);
}

	...
	if (ibp >= limit) {
	  if (traditional) {
	    ...
	    error_with_line (line_for_error (start_line),
			     "unterminated string or character constant", 0, 0, 0);
	  break;
	}
	*obp++ = *ibp;
	switch (*ibp++) {
	...
	for (hp = hashtab[MAKE_POS (hash) % HASHSIZE]; hp != NULL;
	  ...
	  if (hp->length == ident_length) {
	    ...
	    if (disabled) {
		 ...
		 foo(foo, baz)  */
	      if (traditional)
		error ("recursive use of macro `%s'", hp->name, 0, 0);
#endif
 ...
 ending:
  if (if_stack != ip->if_stack) {
    ...
    error_with_line (line_for_error (if_stack->lineno),
		     "unterminated #%s conditional", str, 0, 0);
  }
...
#ifdef ATAC_NOALLOCA
	memptr = cp = (U_CHAR *) xmalloc (bp - buf + 5); /* room for cmd plus
						            some slop */
#else /* ATAC_NOALLOCA */
     ...
     FILE_BUF *op;
{
  ...
  if (ip == NULL) {
    error ("cccp error: not in any file?!", 0, 0, 0);
  ...
  case T_BASE_FILE:
	...
	{
#ifdef ATAC_NOALLOCA
	  buf = (char *) xmalloc (3 + strlen (string));
#else /* ATAC_NOALLOCA */
...
#ifdef ATAC_NOALLOCA
    buf = (char *) xmalloc (8);	/* Eigth bytes ought to be more than enough */
#else /* ATAC_NOALLOCA */
  ...
  case T_VERSION:
#ifdef ATAC_NOALLOCA
    buf = (char *) xmalloc (3 + strlen (version_string));
#else /* ATAC_NOALLOCA */
...
#ifdef ATAC_NOALLOCA
    buf = (char *) xmalloc (4);
    strcpy (buf, " 0 ");	/* Assume symbol is not defined */
#else /* ATAC_NOALLOCA */
...
oops:
    ...
    error ("`defined' must be followed by ident or (ident)", 0, 0, 0);
    break;

  default:
    error ("cccp error: invalid special hash type", 0, 0, 0); /* time for gdb */
    abort ();
  }
  len = strlen (buf);
  check_expand (op, len);
...
get_filename:
  ...
  fail:
    if (retried) {
      error ("#include expects \"fname\" or <fname>", 0, 0, 0);
#ifdef ATAC_NOALLOCA
      if (dsp[0].fname != 0)
        free (dsp[0].fname);
      free (buf);
...
#ifdef ATAC_NOALLOCA
      buf = (U_CHAR *) xmalloc (trybuf.bufp - trybuf.buf + 1);
#else /* ATAC_NOALLOCA */
     ...
     struct directive *keyword;
{
  ...
  if (sym_length == 0)
    error ("invalid macro name", 0, 0, 0);
  else if (!is_idstart[*symname]) {
    U_CHAR *msg;			/* what pain... */
#ifdef ATAC_NOALLOCA
    msg = (U_CHAR *) xmalloc (sym_length + 1);
#else /* ATAC_NOALLOCA */
    msg = (U_CHAR *) alloca (sym_length + 1);
#endif /* ATAC_NOALLOCA */
    bcopy (symname, msg, sym_length);
    msg[sym_length] = 0;
    error ("invalid macro name `%s'", msg, 0, 0);
#ifdef ATAC_NOALLOCA
    free (msg);
#endif /* ATAC_NOALLOCA */
  } else {
    if (! strncmp (symname, "defined", 7) && sym_length == 7)
      error ("defining `defined' as a macro", 0, 0, 0);
  }
  ...
  if (*bp == '(') {
    ...
    while (*bp != ')') {
      ...
      if (!is_idstart[*bp])
	warning ("parameter name starts with a digit in #define", 0, 0, 0);
	...
	error ("badly punctuated parameter list in #define", 0, 0, 0);
#ifdef ATAC_NOALLOCA
	for (temp = arg_ptrs; temp; temp = tempNext) {
	  tempNext = temp->next;
	  free (temp);
        }
#endif /* ATAC_NOALLOCA */
	...
	SKIP_WHITE_SPACE (bp);
      ...
      if (bp >= limit) {
	error ("unterminated parameter list in #define", 0, 0, 0);
#ifdef ATAC_NOALLOCA
	for (temp = arg_ptrs; temp; temp = tempNext) {
	  tempNext = temp->next;
	  free (temp);
...

  {
    ...
    if ((hp = lookup (symname, sym_length, hashcode)) != NULL) {
	  ...
	  char **p;
	  for (p = protected; *p; ++p)
	      if ((*p)[sym_length] == '\0' && strcmp(hp->name, *p) == 0)
		  return 0;
      }
#endif /* ATAC_PROTECTED */
      if (hp->type != T_MACRO
	...
	U_CHAR *msg;			/* what pain... */
#ifdef ATAC_NOALLOCA
	msg = (U_CHAR *) xmalloc (sym_length + 20);
#else /* ATAC_NOALLOCA */
     ...
     struct arglist *arglist;
{
  ...
  if (p + 1 < limit && p[0] == '#' && p[1] == '#') {
    error ("## operator at start of macro definition", 0, 0, 0);
    p += 2;
  }
  ...
  while (p < limit) {
	...
	*exp_p++ = *p++;
	...
	if (p < limit && *p == '#') {
	  ...
	  concat = p;
	  if (limit <= p)
	    error ("## operator at end of macro definition", 0, 0, 0);
	} else {
	  ...
	  if (p == limit || ! is_idstart[*p] || nargs <= 0)
	    error ("# operator should be followed by a macro argument name", 0, 0, 0);
	  else
	    stringify = p;
	}
	break;
     ...
     struct directive *keyword;
{
  ...
  if (!isdigit (*bp)) {
    error ("invalid format #line command", 0, 0, 0);
    return;
  }
     ...
     struct directive *keyword;
{
  ...
  if (! strncmp (buf, "defined", 7) && ! is_idchar[buf[7]])
    warning ("undefining `defined'", 0, 0, 0);

  while ((hp = lookup (buf, -1, -1)) != NULL) {
    if (hp->type != T_MACRO)
      warning ("undefining `%s'", hp->name, 0, 0);
#ifdef ATAC_PROTECTED
    {
	char **p;
	for (p = protected; *p; ++p)
	    if (strcmp(hp->name, *p) == 0) return 0;
    }
#endif /* ATAC_PROTECTED */
    delete_macro (hp);
  }
...
{
  if (pedantic)
    error ("ANSI C does not allow #sccs", 0, 0, 0);
}

/*
     ...
     struct directive *keyword;
{
  ...
  if (if_stack == instack[indepth].if_stack) {
    error ("#elif not within a conditional", 0, 0, 0);
    return;
  } else {
    if (if_stack->type != T_IF && if_stack->type != T_ELIF) {
      error ("#elif after #else", 0, 0, 0);
     ...
     struct directive *keyword;
{
  ...
  if (end == buf) {
    ...
    if (pedantic && buf[0] >= '0' && buf[0] <= '9')
      warning ("#%s argument starts with a digit", keyword->name, 0, 0);
    else if (end != limit && !traditional)
      warning ("garbage at end of #%s argument", keyword->name, 0, 0);
     ...
     int any;
{
  ...
  while (bp < endb) {
	...
	if (strncmp (cp, kt->name, kt->length) == 0
	  ...
	  case T_ELIF:
	    if (if_stack == instack[indepth].if_stack) {
	      error ("#%s not within a conditional", kt->name, 0, 0);
	      break;
	    }
		...
		error ("#else or #elif after #else", 0, 0, 0);
     ...
     struct directive *keyword;
{
  ...
  if (pedantic) {
    SKIP_WHITE_SPACE (buf);
    if (buf != limit)
      warning ("text following #else violates ANSI standard", 0, 0, 0);
  }

  if (if_stack == instack[indepth].if_stack) {
    error ("#else not within a conditional", 0, 0, 0);
    return;
  } else {
    if (if_stack->type != T_IF && if_stack->type != T_ELIF) {
      error ("#else after #else", 0, 0, 0);
      fprintf (stderr, " (matches line %d", if_stack->lineno);
     ...
     struct directive *keyword;
{
  if (pedantic) {
    SKIP_WHITE_SPACE (buf);
    if (buf != limit)
      warning ("text following #endif violates ANSI standard", 0, 0, 0);
  }

  if (if_stack == instack[indepth].if_stack)
    error ("unbalanced #endif", 0, 0, 0);
  else {
     ...
     register U_CHAR *p;
{
  ...
  if (*p && *p != '\n')
    warning ("text following #else or #endif violates ANSI standard", 0, 0, 0);
}

/*
	...
	*eofp = 1;
	...
	++*count_newlines;
 	...
 	break;
      ...
      if (match == '\'') {
	error_with_line (line_for_error (start_line),
			 "unterminated character constant", 0, 0, 0);
	bp--;
	if (eofp)
	  *eofp = 1;
	break;
     ...
     FILE_BUF *op;
{
  ...
  if (nargs >= 0) {
...
#ifdef ATAC_NOALLOCA
    args = (struct argdata *) xmalloc ((nargs + 1) * sizeof (struct argdata));
#else /* ATAC_NOALLOCA */
	...
	= macarg ((i < nargs || (nargs == 0 && i == 0)) ? &args[i] : 0);
      if (parse_error)
	{
	  error_with_line (line_for_error (start_line), parse_error, 0, 0, 0);
	  break;
	}
      i++;
    ...
    if (nargs == 0 && i > 0)
      error ("arguments given to macro `%s'", hp->name, 0, 0);
    else if (i < nargs) {
      /* traditional C allows foo() if foo wants one argument.  */
      if (nargs == 1 && i == 0 && traditional)
	;
      else if (i == 0)
	error ("no args to macro `%s'", hp->name, 0, 0);
      else if (i == 1)
	error ("only 1 arg to macro `%s'", hp->name, 0, 0);
      else
	error ("only %d args to macro `%s'", i, hp->name, 0);
    } else if (i > nargs)
      error ("too many (%d) args to macro `%s'", i, hp->name, 0);
...
#ifdef ATAC_NOALLOCA
    buf = (U_CHAR *) xmalloc (2 * strlen (str) + 1);
#else /* ATAC_NOALLOCA */
